...
log = (str(batchSz) + '_' + str(epoch) + '_' + str(patiences) + '_'  
      ...
      + str(dDp1)+ '_'  + str(dDp2) + '_'  + str(dDp3) + '_' + str(valUse))
...
rating = np.zeros((row, 5))
for i in range(0,row):
    rating[i,(rawData[i,3]-1)] = 1 
user = rawData[:,1]
movie = rawData[:,2]
...
movie_vec = keras.layers.Flatten()(keras.layers.Embedding(n_movies + 1, 64)(movie_input))
#movie_vec = keras.layers.Dropout(0.5)(movie_vec)

user_input = keras.layers.Input(shape=[1])
user_vec = keras.layers.Flatten()(keras.layers.Embedding(n_users + 1, 64)(user_input))
...
result = keras.layers.Dense(5, activation=outAct)(nn)
